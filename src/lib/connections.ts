// Shared "Connections" data model — gathers directly related objects for a
// given project object (used by ConnectionsPanel and reused by TruthMirrorPanel
// so both draw on the same relationship graph and navigate the same way).
import { useMemo } from 'react';
import { findItem, useAppStore } from '../store/appStore';
import type { AppArea, BinderItem, ObjectType } from '../types';

export type ConnectionType = Exclude<ObjectType, 'revision_pass' | 'manuscript_assembly'>;
export interface Connection { id: string; type: ConnectionType; title: string; subtitle?: string }

export const GROUPS: Record<ConnectionType, { label: string; area: AppArea }> = {
...
export function useConnections(objectType: ConnectionType, objectId: string): Connection[] {
...
// Same jump behaviour Connections links use, exposed so other panels (e.g.
// Truth Mirror's "related objects to review") can navigate consistently.
export function navigateToConnection(state: ReturnType<typeof useAppStore.getState>, c: Connection) {
